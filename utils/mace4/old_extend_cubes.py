...
def run_process(id, slot_id, thread_slots, order, cube_length, input_file, cube, print_models, working_dir):
...
def extend_cube(input_file, order, cube_length, cubes, print_models, working_dir, max_threads):
    """ 
    ...
    for cube in all_cubes:
        ...
        thread_slots[slot_id] = threading.Thread(target=run_process, args=(id, slot_id, thread_slots, order, cube_length, f"../{input_file}", seq, print_models, working_dir))
...
if __name__ == "__main__":
    ...
    extend_cube(f"inputs/{algebra}.in", order, new_cube_length, f"utils/mace4/working/{algebra}{order}/cubes_2_{order}_{cube_length}.out",
             print_models, f"{algebra}_working{order}", 8)
