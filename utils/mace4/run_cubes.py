...
def run_process(id, slot_id, thread_slots, order, input_file, cube, working_dir):
    ...
    subprocess.run(f"cd {working_dir}; {mace4} -n{order} -N{order} -m-1 -A1 -O3 -f {input_file} >> mace.log 2>&1", 
...
def run_mace(input_file, order, cubes, working_dir, max_threads):
    ...
    for cube in all_cubes:
        ...
        thread_slots[slot_id] = threading.Thread(target=run_process, args=(id, slot_id, thread_slots, order, f"../{input_file}", seq, working_dir))
...
if __name__ == "__main__":
    ...
    run_mace(f"inputs/{algebra}.in", order, f"utils/mace4/{algebra}{order}/cubes_2_{order}_{cube_length}.out", f"{algebra}_working", 8)
